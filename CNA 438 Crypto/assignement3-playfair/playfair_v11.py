##Dustin Watson
##CNA 438
##Playfair
##Heavy use of Typesafe checking
##Program will error out with negitive returns if something is out of bounds or unexpected.$
##Each function uses mutual suspicion

import math
import re
from random import *
import string
from tkinter import *
from tkinter import messagebox



class Window(Frame):

    def __init__(self, master=None):
        Frame.__init__(self, master)               
        self.master = master
        self.init_window()

    def init_window(self):
        self.master.title("PlayFair")

        #Define all stringVars for use with the GUI:
        self.PTStr = StringVar()
        self.CTStr = StringVar()
        self.KeywordStr = StringVar()
        self.OutputStr = StringVar()
        self.table = ""

        #Keyword entery and label
        self.Keyword_Label = Label(self.master, text="Keyword:")
        self.Keyword_Label.grid(row=0,column=0,pady=2,padx=2)
        
        self.Keyword = Entry(self.master, textvariable=self.KeywordStr)
        self.Keyword.insert(INSERT, "keyword")
        self.Keyword.grid(row=0,column=1,pady=2,padx=2)

        self.outputlbl = Label(self.master, text="Output:")
        self.outputlbl.grid(row=3, column=0)

        self.outputbox = Entry(self.master, textvariable=self.OutputStr)
        self.outputbox.grid(row=3,column=1)
        
        self.PT_Label = Label(self.master, text="PlainText:")
        self.PT_Label.grid(row=1,column=0, pady=2,padx=2)
        
        self.PT = Entry(self.master, textvariable=self.PTStr)
        self.PT.insert(INSERT, "Text to Encode")
        self.PT.grid(row=1,column=1, pady=2,padx=2)
        
        self.CT_Label = Label(self.master, text="CipherText:")
        self.CT_Label.grid(row=2,column=0, pady=2,padx=2)
        
        self.CT = Entry(self.master, textvariable=self.CTStr)
        self.CT.grid(row=2,column=1, pady=2,padx=2)
        
        self.Encrypt_butt = Button(self.master, text="Encrypt", command=self.start_encrypt)
        self.Encrypt_butt.grid(row=1,column=2, pady=2,padx=2)
        
        self.Dencrypt_butt = Button(self.master, text="Decrypt", command=self.start_decrypt)
        self.Dencrypt_butt.grid(row=2,column=2, pady=2,padx=2)

        self.CipherBlocksLbl = Label(self.master, text="Cipher Blocks")
        self.CipherBlocksLbl.grid(row=0, column=3, pady=2,padx=2)

        self.CipherBlocks = Listbox(self.master)
        self.CipherBlocks.grid(row=1, column=3, pady=2,padx=2, rowspan=3)

        self.PTBlocksLbl = Label(self.master, text="PT Blocks")
        self.PTBlocksLbl.grid(row=0, column=4, pady=2,padx=2)

        self.PTBlocks = Listbox(self.master)
        self.PTBlocks.grid(row=1, column=4, pady=2,padx=2, rowspan=3)

        self.ShowTable = Button(self.master, text="Show Table", command=self.show_table)
        self.ShowTable.grid(row=6, column=1, pady=2,padx=2, rowspan=3)


    def show_table(self):
        if self.table == "":
            messagebox.showwarning("Warning", "No table created yet")
        else:
            messagebox.showwarning("Table 6x6", self.table)

    def start_encrypt(self):
        if self.PTStr.get() == "" or self.KeywordStr.get() == "":
            messagebox.showwarning("Warning", "Keyword or PlainText is blank. Correct this and try again")
            return
        else:
            #Check for invalid chars in keyword and plainText Boxes
            if(invalidchars_exist(self.PTStr.get())):
                messagebox.showwarning("Warning", "Plain Text has invalid charactors. If one isn't visiable, check for spaces. Must be a-z 0-9 Correct this and try again")
                return
            if(invalidchars_exist(self.KeywordStr.get())):
               messagebox.showwarning("Warning", "Keyword has invalid charactors. If one isn't visiable, check for spaces. Must be a-z 0-9 Correct this and try again")
               return
            if(repeats_exist(self.KeywordStr.get())):
                messagebox.showwarning("Warning", "Keyword has duplicate charactors. Correct this and try again")
                return
            #Now all the checks for TypeSafe are done, compute the output
            #Enter Output code here:
            self.OutputStr = "" #clear out the output box
            print(self.KeywordStr.get())
            playfairlist = playfair_table(self.KeywordStr.get()) #create the encryption table
            #set the table as a string to pass to display it:
            self.table = ""
            counter = 0
            for letters in playfairlist:
                if (counter % 6) == 0:
                    self.table = self.table +  chr(13)
                self.table = self.table + letters
                counter += 1
            print(playfairlist)
            print(self.table)

            messagedict = playfair_message_blocks(self.PTStr.get(),playfairlist) #use that table to set up a list

            print(messagedict)
            print("MessageDict List blocks:")
            print(messagedict["list"])
            #delete all the items in the list box
            self.PTBlocks.delete(0, END)
            for block in messagedict["list"]:
                self.PTBlocks.insert(END, block[0]+block[1])
            
            CT = encrypt_pf(messagedict, playfairlist) #use the table and grouped blocks to genreate a Ciphertext
            self.OutputStr = CT
            
            #self.OutputStr = avc(KeysList, self.PTStr.get(), self.KeywordStr.get())
            self.outputbox.delete(0,END)
            self.outputbox.insert(0, self.OutputStr)

            
    def start_decrypt(self):
        if self.CTStr.get() == "" or self.KeywordStr.get() == "":
            messagebox.showwarning("Warning", "Keyword or Cipher Text is blank. Correct this and try again")
            return
        else:
            #Check for invalid chars in keyword and plainText Boxes
            if(invalidchars_exist(self.CTStr.get())):
                messagebox.showwarning("Warning", "Cipher Text has invalid charactors. If one isn't visiable, check for spaces. Must be a-z 0-9 Correct this and try again")
                return
            if(invalidchars_exist(self.KeywordStr.get())):
               messagebox.showwarning("Warning", "Keyword has invalid charactors. If one isn't visiable, check for spaces. Must be a-z 0-9 Correct this and try again")
               return
            if(repeats_exist(self.KeywordStr.get())):
                messagebox.showwarning("Warning", "Keyword has duplicate charactors. Correct this and try again")
                return

            #Now all the checks for TypeSafe are done, compute the output
            #Enter Output code here:
            self.OutputStr = "" #clear out the output box

            playfairlist = playfair_table(self.KeywordStr.get())
            self.table = ""
            counter = 0
            for letters in playfairlist:
                if (counter % 6) == 0:
                    self.table = self.table +  chr(13)
                self.table = self.table + letters
                counter += 1

            messagedict = playfair_message_blocks(self.CTStr.get(),playfairlist)
            CT = decrypt_pf(messagedict, playfairlist)
            self.OutputStr = CT
            self.CipherBlocks.delete(0, END)
            for block in messagedict["list"]:
                self.CipherBlocks.insert(END, block[0]+block[1])
                        
            #self.OutputStr = deavc(KeysList, self.CTStr.get(), self.KeywordStr.get())
            self.outputbox.delete(0,END)
            self.outputbox.insert(INSERT, self.OutputStr)


def pf_rules(offset, messageblock):
    if not isinstance(offset, int):
        print("Error (pf_rules): offset is not a Integer")
        return -1
##    if messagedict == "":
##        print("Error (pf_rules): messageblock Input is not a Dict type")
##        return -2
    #Rule 0:
    #if both letters in the block are XX then do nothing.
    #Assumes the following ways XX happens:
    #Ends in a X and X is added to the end.
    #XX and an X is inserted to create XXX, then XX XY is split into two blocks.
    if messageblock[0] == messageblock[1]:
        return []
    #Rule 1:
    if same_col(messageblock[0], messageblock[1]):
        return [change_row(messageblock[0],offset,6), change_row(messageblock[1],offset,6)]
    ##Rule 2:
    if same_row(messageblock[0], messageblock[1]):
        return [change_col(messageblock[0],offset,6), change_col(messageblock[1],offset, 6)]
    ##Rule 3:
    if not same_row(messageblock[0], messageblock[1]) and not same_col(messageblock[0], messageblock[1]):
        return swap_cols(messageblock[0], messageblock[1])
    
 

def invalidchars_exist(stringvalue):
    if not isinstance(stringvalue, str):
        print("Error (invalidchars_exist): Input is not a string")
        return -1
    stringvalue = stringvalue.lower()
    alphanum = string.ascii_lowercase + string.digits
    location = 0
    for char in stringvalue:
        location = alphanum.find(char)
        if location == -1:
            return True
    return False

def repeats_exist(stringvalue):
    if not isinstance(stringvalue, str):
        print("Error (repeats_exist): Input is not a string")
        return -1
    stringvalue= stringvalue.lower()
    for i in range(0, len(stringvalue)-1):
        for j in stringvalue[i+1:len(stringvalue)]:
            if stringvalue[i] == j:
                return True
    return False

def playfair_table(keyword):
    if not isinstance(keyword, str):
        print("Error (playfair_table): Input is not a string")
        return -3
    #check keyword for invalid charactors
    if invalidchars_exist(keyword):
        print("Error (playfair_table): invalid Chars in PT")
        return -2
    #check for repeats in keyword
    if repeats_exist(keyword):
        print("Error (playfair_table): Repeats exist")
        return -1
    #keyword is string
    keyword = keyword.lower()
    charset = string.ascii_lowercase + string.digits
    #create a keyword list
    completedkeyword = list()
    remainingcharset = list(charset)
    for char in keyword:
        completedkeyword = completedkeyword + [remainingcharset.pop(remainingcharset.index(char))]
    completedkeyword = completedkeyword + remainingcharset #add all the reset of the 36 alphadigit to the keyword
    return completedkeyword

def playfair_message_blocks(message, table, removespaces=True):
    #Rules of PF
    #1. Must be split into PAIRS
    #2. Seperate all duplicated leters by inserting letter X
    #3. If there is an odd letter at the end of message, insert X
    #4. ignore all spaces
    ##Rule 1:
    #check if message is a string
    if not isinstance(message, str):
        print("Error (playfair_message_blocks): Input is not a string")
        return -1
    #check keyword for invalid charactors
    message = message.lower()
    if removespaces is True:
        message = message.replace(" ","") #Rule 4
    elif invalidchars_exist(message):
        print("Error (playfair_message_blocks): invalid Chars in message")
        return -2
    messagelist = [x for x in message]
    i=0
    while i < len(messagelist)-1:
        if messagelist[i] == messagelist[i+1]: ##Rule 2
            messagelist.insert(i+1, "x")
            i += 1
        i += 1
    if len(messagelist) % 2 != 0: #Rule 3:
        messagelist.append("x")
    #Rule 1:
    newmessagelist = [messagelist[i*2:(i*2)+2] for i in range(0,(len(messagelist)//2))]
    #decomp the list back into a string:
    newmessagestr = ""
    for char in messagelist:
        newmessagestr += char
    ##if table is set, then also output a table comp of the message
    ##using a list of pairs into tables index numbers matching the letters.
    if table is not None:
        tablecomp = []
        if not isinstance(table, list):
            print("Error (playfair_message_blocks): Table Input is not a List")
            return -4
        #convert each 2 char block into numbers from the table
        for block in newmessagelist:
            currentblocklist = []
            for letter in block:
                location = table.index(letter)
                currentblocklist.append(location)
            tablecomp.append(currentblocklist)
        return {"list": newmessagelist, "string": newmessagestr, "block_int":tablecomp}
    else:
        return {"list": newmessagelist, "string": newmessagestr}

def encrypt_pf(messagedict, playfair_table_list):
##    Rule 1:
##    If in the same Column:
##        Move each letter down one row (increase the row 1), wrap around is used.
##    Rule 2:
##    If in the same Row:
##        Move each letter right one colum (increase the colum 1), wrap around is used.
##    Rule 3:
##    If not rule 1 or 2, then swap the letter's columns
##
## Input messagedict as dict type
## Input play_table_list as list
    if not isinstance(messagedict,dict):
        print("Error (encrypt_pf): messagedict Input is not a Dict type")
        return -1
    if not isinstance(playfair_table_list,list):
        print("Error (encrypt_pf): playfair_table_list Input is not a list type")
        return -2 
    block_int = messagedict["block_int"]
    #create 6x6 table using duple values (x,y) to set the cols, rows.
    #retain the 2 blocks as well.
    table = [] #init the duple
    for block in block_int:
        temptable=[]
        for letter in block:
            temptable.append((letter%6,letter//6))
        table.append(temptable)
    newblocks = []
    newtable = []
    tempblock = []
    print("Apply rules for encrypting")
    for block in table:
        newtable.append(pf_rules(1, block)) #Apply pf rules moving the rows, cols by 1
    #now convert the new duple values into letters on keyword table
    #playfair_table_list
    print(newtable)
    flattentablelist= []
    for block in newtable:
        for letter in block:
            flattentablelist.append((letter[1]*6)+letter[0]) #row * 6 + col (col is equvlent to remainder and row is the quoatent
    CToutputstring = ""
    for CTLetterInt in flattentablelist: #find them on the playfair table list to make an CT
        CToutputstring +=  playfair_table_list[CTLetterInt]
    return CToutputstring

def same_col(dup1, dup2):
    if dup1[0] == dup2[0]:
        return True
    else:
        return False

def same_row(dup1, dup2):
    if dup1[1] == dup2[1]:
        return True
    else:
        return False

def swap_cols(dup1, dup2):
    print("Rule3")
    #function swaps the cols of 2 duple values
    #returns two dupl with their cols swapped as a list
    return [(dup2[0], dup1[1]), (dup1[0],dup2[1])]

def change_col(dup1, offset, x):
    print("Rule2")
    #Function increase the row of a dup by 1 with wraparound x
    #Returns: dup
    newdup = ((dup1[0] + offset) % x, dup1[1])
    return newdup


def change_row(dup1, offset, x):
    print("Rule1")
    #Function increase the row of a dup by 1 with wraparound x
    #Returns: dup
    newdup = (dup1[0], (dup1[1] + offset) % x)
    return newdup

def decrypt_pf(messagedict, playfair_table_list):
##    Rule 1:
##    If in the same Column:
##        Move each letter up one row (decrase row), wrap around is used.
##    Rule 2:
##    If in the same Row:
##        Move each letter left one col (decrase col), wrap around is used.
##    Rule 3:
##    If not rule 1 or 2, then swap the letter's columns, leave the rows unchanged.
##
## Input messagedict as dict type
## Input play_table_list as list
    if not isinstance(messagedict,dict):
        print("Error (decrypt_pf): messagedict Input is not a Dict type")
        return -1
    if not isinstance(playfair_table_list,list):
        print("Error (decrypt_pf): playfair_table_list Input is not a list type")
        return -2 
    block_int = messagedict["block_int"]
    #create 6x6 table using duple values (x,y) to set the cols, rows.
    #retain the 2 blocks as well.
    table = [] #init the duple
    #create a visualization table, if I have the time, animate the letters moving
    vis_table = []
    for block in block_int:
        temptable=[]
        for letter in block:
            temptable.append((letter%6,letter//6))
            vis_table.append((letter%6,letter//6))
        table.append(temptable)
    newblocks = []
    newtable = []
    tempblock = []
    print("apply rules for decrypt")
    for block in table:
        newtable.append(pf_rules(-1, block)) #Apply pf rules moving the rows, cols by 1
    #now convert the new duple values into letters on keyword table
    #playfair_table_list
    flattentablelist= []
    for block in newtable:
        for letter in block:
            flattentablelist.append((letter[1]*6)+letter[0]) #row * 6 + col (col is equvlent to remainder and row is the quoatent
    CToutputstring = ""
    for CTLetterInt in flattentablelist: #find them on the playfair table list to make an CT
        CToutputstring +=  playfair_table_list[CTLetterInt]
    return CToutputstring
       
##playfairlist = playfair_table("40azyxv1")
##
##PT1="Superman never made anty"
####never made any money\
##Saving the world from Solomon Grundy \
##And sometimes I despair \
##The world will never see another man like him"

##print("PT1:", PT1)     
##messagedict = playfair_message_blocks(PT1,playfairlist)
##print(messagedict["list"])
##CT = encrypt_pf(messagedict, playfairlist)
##
##print("CT:", CT)
##
##PT2 = decrypt_pf(playfair_message_blocks(CT,playfairlist), playfairlist)
##print("PT:", PT2)

root = Tk()
root.geometry("550x250")
root.maxsize(width=550, height=250)
root.minsize(width=550, height=250)
app = Window(root)

root.mainloop()


